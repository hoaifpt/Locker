...
public class OrderService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        slot.Status = LockerSlotStatus.Reserved;
        slot.BookingId = order.Id;
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var slot in locker.Slots.Where(s => s.Status == LockerSlotStatus.Available))
        {
            ...
            {
                var package = await _packageRepository.GetByIdAsync(slot.PackageId, cancellationToken);
                if (package != null)
                {
                    availableSlots.Add(new AvailableSlotDto
                    {
                        ...
                        PricePerHour = package.PricePerHour
                    });
                }
            }
        }
